...
export async function handleAdminFlow(ctx, res) {
  ...
  const mapaDelMundo = {
    ...
    planificacion_bloques_pod: { id: "UUID", fecha: "DATE", equipo_id: "UUID REFERENCES equipos(id)", hora_inicio: "TIME", hora_fin: "TIME", especialidad_id: "UUID REFERENCES especialidades(id)", supervisor_id: "UUID REFERENCES personal(id)", actividad_especifica: "TEXT", creado_at: "TIMESTAMP" }
  };
...
9. NOTAS DE DATOS Y COLUMNAS:
   ...
   - **PLANIFICACIÓN MATUTINA / SALA POD:** Si el supervisor te solicita registrar o programar una asignación o bloqueo de equipo para mañana o una fecha específica (por ejemplo, "mañana el camión pluma CAPL-0029 tiene mantención todo el día" o "el equipo X tiene taller de 08:00 a 12:00 mañana"), debes crear y ejecutar una herramienta dinámica que realice un INSERT en la tabla 'planificacion_bloques_pod'.
     - Si el supervisor dice "todo el día", el rango de horas debe ser desde '07:00:00' hasta '18:00:00'.
     - Para programar "Mantención" (Preventiva/Programada), usa el ID de supervisor virtual '11111111-1111-1111-1111-111111111111' y la actividad_especifica "Mantenimiento Preventivo".
     - Para programar "Taller" o "Reparación", usa el ID de supervisor virtual '22222222-2222-2222-2222-222222222222' y la actividad_especifica "Taller / Reparación".
     - En ambos casos de mantenimiento/reparación, busca el ID de especialidad cuyo nombre sea 'Mantenimiento' e insértalo en 'especialidad_id'.
